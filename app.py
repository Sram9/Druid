...
if 'focus_coords' not in state: state.focus_coords = None
...
if state.page=='archives':
    ...
    order = st.radio("Trier par :", ["Nom","Date"], horizontal=True)
    ...
    for i,p in enumerate(sorted_archives):
        with st.expander(f"{p['nom']} ({p['date'][:10]})"):
            ...
            if c1.button("📍 Localiser", key=f"loc{i}"):
                ...
                state.focus_coords = p.get('coords')
                state.page = 'map'
...
if state.page=='map':
    ...
    if archives:
        df = pd.DataFrame([
            {'lat': float(p['coords'].split(',')[0]), 'lon': float(p['coords'].split(',')[1]), 'nom': p['nom']}
            for p in archives if p.get('coords')
        ])
        if not df.empty:
            st.map(df)
            if state.focus_coords:
                lat,lon = map(float, state.focus_coords.split(","))
                st.success(f"Focus sur {state.selected_name} en {state.focus_coords}")
                state.focus_coords = None
    else:
        st.info("Pas encore de plantes localisées.")
    st.stop()
...
if state.page=='home':
    ...
    if up:
        ...
        try:
            ...
            st.warning("PlantNet a échoué, tentative avec Plant.id")
        ...
        st.write(v)
        q = st.text_input("❓ Poser une question sur cette plante")
        if q:
            now=datetime.utcnow()
            state.mistral_calls=[t for t in state.mistral_calls if now-t<timedelta(seconds=60)]
            if len(state.mistral_calls)<3:
                body={"model":"mistral-tiny","messages":[{"role":"user","content":f"Concernant {name}, {q}"}], "max_tokens":200}
                h={"Authorization":f"Bearer {MISTRAL_API_KEY}","Content-Type":"application/json"}
                j=requests.post("https://api.mistral.ai/v1/chat/completions",headers=h,json=body).json()
                ...
                st.warning("Limite d'appels atteinte. Patientez 1 minute.")
