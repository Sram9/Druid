...
if state.page == 'archives':
    ...
    for i, p in enumerate(sorted_archives):
        with st.expander(f"{p['nom']} ({p['date'][:10]})"):
            ...
            # Préparer points GPS si existants
            coords = p.get('coords')
            if coords:
                try:
    ...
    if gps_points:
        st.markdown("---")
        st.subheader("🗺️ Carte de toutes les plantes localisées")
        df = pd.DataFrame(gps_points)
        st.map(df)
    else:
        st.info("Aucune plante localisée pour le moment.")

    if state.show_map:
        st.markdown("---")
        st.subheader(f"🧭 Localisation de : {state.selected_name}")
        if state.selected_coords:
            try:
                lat, lon = map(float, state.selected_coords.split(','))
                st.map(pd.DataFrame([{'lat': lat, 'lon': lon}]))
                link = f"https://www.google.com/maps/dir/?api=1&destination={lat},{lon}"
                st.markdown(f"[🧭 Démarrer la navigation vers {state.selected_name}]({link})")
            except:
...
if state.page == 'search':
    ...
    if keyword:
        ...
        if results:
            ...
            st.info(f"Aucune plante trouvée avec le mot-clé '{keyword}'.")
    if st.button("🔙 Retour archives"):
        state.page = 'archives'
...
if state.page == 'home':
    ...
    if up:
        ...
        with st.spinner('🕵️‍♂️ Identification de la plante en cours...'):
            try:
                resp = requests.post(
        ...
        # Récupération des vertus via Mistral
        ...
        if name in cache:
            ...
            if len(state.mistral_calls) < 3:
                body = {
                    "model": "mistral-tiny",
                    "messages": [{"role": "user", "content": f"Cette plante ({name}) est-elle comestible ? A-t-elle des vertus médicinales ? Si oui, comment est-elle utilisée ?"}],
                    "max_tokens": 400
                }
                ...
                v = "Limite de requêtes atteinte. Veuillez patienter."
        ...
        if st.button("✅ Archiver cette plante"):
            with st.spinner('Archivage...'):
                archives.append({
                    ...
                    "coords": state.coords,  # Peut être None si GPS échoué
                    "vertus": v
                })
                open(ARCHIVES_PATH, 'w', encoding='utf-8').write(json.dumps(archives, ensure_ascii=False, indent=2))
                st.toast("Plante archivée ! 🌿")
            st.success("Archivée avec succès ✅")
